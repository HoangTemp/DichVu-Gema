...
$orderConditon = "";
//Tìm kiếm
$search = isset($_GET['maloai']) ? $_GET['maloai'] : "";
...
if ($search) {
    $products = mysqli_query($conn, "SELECT * FROM `sanpham` WHERE `maloai` LIKE '%" . $search . "%' " . $orderConditon . "  LIMIT " . $item_per_page . " OFFSET " . $offset);
    $totalRecords = mysqli_query($conn, "SELECT * FROM `sanpham` WHERE `maloai` LIKE '%" . $search . "%'");
} else {
